...
def test_cli_stylesheet(qapp, qtbot, happi_cfg):
    ...
    try:
        ...
        window = typhos_cli(["test_motor", "--stylesheet", "test.qss", "--happi-cfg", happi_cfg])
        qtbot.addWidget(window)
...
@pytest.mark.parametrize(
    "klass, name", [("ophyd.sim.SynAxis[]", "SynAxis"), ("ophyd.sim.SynAxis[{'name':'foo'}]", "foo")]
)
...
def test_cli_profile_modules(capsys, qtbot):
    window = typhos_cli(["ophyd.sim.SynAxis[]", "--profile-modules", "typhos.suite"])
    qtbot.addWidget(window)
...
def test_cli_profile_output(capsys, qtbot):
    ...
    window = typhos_cli(["ophyd.sim.SynAxis[]", "--profile-output", str(path_obj)])
    qtbot.addWidget(window)
    output = capsys.readouterr()
