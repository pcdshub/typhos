...
class MyDevice(ophyd.Device):
    ...
    command_setpoint_tracks_readback = Cpt(EpicsSignal, "command-setpoint-tracks-readback")
    set_metadata(command_setpoint_tracks_readback, {"variety": "command-setpoint-tracks-readback"})
    ...
    set_metadata(
        bitmask,
        {
            "variety": "bitmask",
            "bits": 4,
            "style": dict(shape="circle", on_color="yellow", off_color="white"),
            "meaning": ["A", "B", "C"],
        },
    )
...
class VarietyIOC(PVGroup):
    ...
    command_with_enum = pvproperty(
        value=0, name="command-with-enum", enum_strings=["Off", "On"], dtype=caproto.ChannelType.ENUM
    )

    command_setpoint_tracks_readback = pvproperty(value=0, name="command-setpoint-tracks-readback")
    tweakable = pvproperty(
        value=0,
        name="tweakable",
        lower_ctrl_limit=-5,
        upper_ctrl_limit=5,
    ...
    enum = pvproperty(value="enum1", enum_strings=["enum1", "enum2"], dtype=caproto.ChannelType.ENUM, name="enum")


if __name__ == "__main__":
    ioc_options, run_options = ioc_arg_parser(default_prefix="variety:", desc=dedent(VarietyIOC.__doc__))
