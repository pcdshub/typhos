...
def func_display(qtbot):
    ...
    def foo(first, second: float = 3.14, hide: bool = True, third=False):
        kwargs.update({"first": first, "second": second, "hide": hide, "third": third})

    # Create display
    func_dis = FunctionDisplay(foo, annotations={"first": int}, hide_params=["hide"])
...
def test_func_display_creation(func_display, qtbot):
    ...
    assert "hide" not in [widget.parameter for widget in func_display.param_controls]
    # Check that we sorted our parameters correctly
    assert "first" in func_display.required_params
    assert all([key in func_display.optional_params for key in ["second", "third"]])
    return func_display
