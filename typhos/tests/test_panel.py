...
from ophyd.sim import FakeEpicsSignal, FakeEpicsSignalRO, SynSignal, SynSignalRO
from pydm.widgets import PyDMEnumComboBox
from qtpy.QtWidgets import QWidget

from typhos import cache, utils
from typhos.panel import SignalPanel, TyphosSignalPanel
from typhos.widgets import ImageDialogButton, WaveformDialogButton, create_signal_widget
...
def wait_panel(qtbot, panel, signal_names):
    ...
    blocker = qtbot.wait_signal(panel.loading_complete, check_params_cb=condition)
    blocker.wait()
...
def test_panel_creation(qtbot, panel, panel_widget):
    ...
    read_and_write = FakeEpicsSignal("Tst:Read", write_pv="Tst:Write", name="read_and_write")
    ...
    wait_panel(qtbot, panel, signal_names={sig.name for sig in signals.values()})
...
@pytest.mark.xfail(reason="PVs do not exist so widgets are not created post refactor")
...
def test_typhos_panel_sort_by_name(qapp, client, qtbot, typhos_signal_panel):
    ...
    qtbot.wait_until(have_device)
    device = panel.devices[0]
    sorted_names = [getattr(device, attr).name for attr in sorted(device.component_names)]
